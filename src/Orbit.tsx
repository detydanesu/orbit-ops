...
type ResourceData = {
  ...
  publicKey?: string; keyFingerprint?: string; sshAuthMethod?: 'key' | 'password' | ''; hasSshCredentials?: boolean; hasPrivateKey?: boolean; hostFingerprint?: string; connectionStatus?: string; lastCheckedAt?: string
}
...
type Draft = { type: ResourceType; name: string; host: string; port: string; username: string; provider: string; endpoint: string; location: string; notes: string; parentId: string }
...
const blankDraft: Draft = { type: 'vps', name: '', host: '', port: '22', username: '', provider: '', endpoint: '', location: '', notes: '', parentId: '' }
...
function SignIn({ onSuccess }: { onSuccess: () => void }) {
  ...
  return <main className="signin-screen"><form className="signin-card" onSubmit={submit}>
    ...
    {import.meta.env.DEV && <div className="dev-warning">Local development mode uses a fixed encryption key. Do not save real SSH credentials here.</div>}
    ...
    <div className="signin-foot"><ShieldCheck size={14} /> SSH credentials are encrypted on this server.</div>
  </form></main>
}
...
function App() {
  ...
  const openAdd = () => {
    ...
    setEditingNodeId(''); setDraft(blankDraft); setSelectedIdentityId(''); setSshAuthMethod('key'); setSshPassword(''); setMessage(null); restoreWindow('add'); setAddOpen(true)
    void refreshSshLibrary().catch((error) => notify(error instanceof Error ? error.message : 'Could not load saved SSH setups.', 'error'))
  }
  const closeAdd = () => { if (!saveBusy) { setAddOpen(false); setSshPassword('') } }
  ...
  const uploadSshIdentity = async (name: string, privateKey: string, passphrase: string, uploadFileName: string) => {
    await request<{ identity: StoredSshIdentity }>('/ssh-identities', { method: 'POST', body: JSON.stringify({ name, privateKey, passphrase, uploadFileName }) })
    await refreshSshLibrary()
    notify('Encrypted SSH key saved to the library.')
  }
  ...
  const switchBoard = async (id: string) => {
    ...
    setDetailsOpen(false); setAddOpen(false); setEditOpen(false); setSshPassword(''); setSshAuthMethod('key'); setBoardDialog(null); setVerification(null); setDeleteConfirm(false)
  ...
  const addNode = async (event: FormEvent) => {
    ...
    if (draft.type === 'vps' && sshAuthMethod === 'key' && !sshIdentities.some((identity) => identity.id === selectedIdentityId)) { notify('Upload a private key in SSH Files, then select it here.', 'error'); return }
    if (draft.type === 'vps' && sshAuthMethod === 'password' && !sshPassword) { notify('Enter the SSH password for this host.', 'error'); return }
    setSaveBusy(true)
    const x = 90 + (realNodes.length % 3) * 278
    const y = 110 + (Math.floor(realNodes.length / 3) % 2) * 205
    const body = { ...draft, sshAuthMethod: draft.type === 'vps' ? sshAuthMethod : undefined, sshPassword: draft.type === 'vps' && sshAuthMethod === 'password' ? sshPassword : undefined, sshIdentityId: draft.type === 'vps' && sshAuthMethod === 'key' ? selectedIdentityId : undefined, boardId, port: Number(draft.port || 22), x, y, parentId: draft.parentId || undefined }
    try {
      await request('/nodes', { method: 'POST', body: JSON.stringify(body) })
      if (draft.type === 'vps') await refreshSshLibrary()
      setDraft(blankDraft); setSshPassword(''); setSshAuthMethod('key'); setAddOpen(false); setVerification(null); await loadGraph(); notify('Resource added to the map.')
  ...
  const openEdit = () => {
    ...
    setDraft({ type: data.resourceType, name: data.name, host: data.host || '', port: String(data.port || 22), username: data.username || '', provider: data.provider || '', endpoint: data.endpoint || '', location: data.location || '', notes: data.notes || '', parentId: '' })
  ...
  const saveEdit = async (event: FormEvent) => {
    ...
    setSaveBusy(true); setMessage(null)
    const body = draft.type === 'vps'
      ? { name: draft.name, host: draft.host, port: Number(draft.port || 22), username: draft.username, location: draft.location, notes: draft.notes }
      : { name: draft.name, provider: draft.provider, endpoint: draft.endpoint, notes: draft.notes }
    try {
      await request('/nodes/' + encodeURIComponent(editingNodeId), { method: 'PATCH', body: JSON.stringify(body) })
      setEditOpen(false); setSelectedId(editingNodeId)
      await loadGraph()
      if (draft.type === 'vps') await refreshSshLibrary()
      notify('Resource updated.')
    } catch (error) { notify(error instanceof Error ? error.message : 'Could not update this resource.', 'error') }
    finally { setSaveBusy(false) }
  }
  ...
  const signOut = async () => {
    ...
    ++graphRequest.current; boardRef.current = 'default'; setBoardId('default'); setBoards([]); setSshIdentities([]); setSshFiles([]); setSshProfiles([]); setSelectedIdentityId(''); setSshPassword(''); setSshAuthMethod('key'); setSshLibraryOpen(false); setDetailsOpen(false); setAddOpen(false); setTerminalNode(null); setBoardDialog(null); setMinimizedWindows([])
  ...
  const openWindows = [
    ...
    ...(editOpen ? [{ id: 'edit', title: 'Edit resource', minimized: minimizedWindows.includes('edit') }] : []),
  ...
  return <ReactFlowProvider><div className={desktop ? 'app-frame desktop-mode' : 'app-frame'}>
    ...
    {addOpen && <WindowFrame desktop={desktop} title="Add connection" onClose={closeAdd} focusToken={windowFocus.add || 0} minimized={minimizedWindows.includes('add')} onMinimize={() => minimizeWindow('add')}><form className="resource-modal" onSubmit={(event) => void addNode(event)}><header className="modal-heading"><div><span className="modal-kicker">MAP INVENTORY</span><h2>Add a resource</h2><p>Add to {activeBoard?.name || 'this graph'}.</p></div><button type="button" className="modal-close" onClick={closeAdd} aria-label="Close"><X size={17} /></button></header>
      <div className="type-switch" role="group" aria-label="Resource type"><button type="button" className={draft.type === 'vps' ? 'selected' : ''} onClick={() => { setSelectedIdentityId(''); setDraft({ ...blankDraft, type: 'vps' }) }}><Server size={15} /> VPS / SSH</button><button type="button" className={draft.type !== 'vps' ? 'selected' : ''} onClick={() => { setSshPassword(''); setSshAuthMethod('key'); setSelectedIdentityId(''); setDraft({ ...draft, type: draft.type === 'vps' ? 'service' : draft.type }) }}><Layers3 size={15} /> SERVICE</button></div>
      {draft.type === 'vps' && <SshConfigImport savedProfiles={sshProfiles} onOpenLibrary={() => void openSshLibrary()} onSelect={(profile) => { setDraft((current) => ({ ...current, name: profile.alias, host: profile.host, port: String(profile.port), username: profile.username })); notify('Saved host settings loaded. Choose an SSH key or password.'); }} />}
      ...
      {draft.type === 'vps' ? <><div className="field-row"><label className="field-label" htmlFor="ssh-host">Host / IP<input id="ssh-host" className="text-input" placeholder="203.0.113.10" value={draft.host} onChange={(event) => setDraft({ ...draft, host: event.target.value })} autoComplete="off" required /></label><label className="field-label port-field" htmlFor="ssh-port">Port<input id="ssh-port" className="text-input" inputMode="numeric" value={draft.port} onChange={(event) => setDraft({ ...draft, port: event.target.value })} required /></label></div><div className="field-row"><label className="field-label" htmlFor="ssh-user">SSH username<input id="ssh-user" className="text-input" placeholder="root" value={draft.username} onChange={(event) => setDraft({ ...draft, username: event.target.value })} autoComplete="off" required /></label><label className="field-label" htmlFor="ssh-location">Location <span className="optional-label">OPTIONAL</span><input id="ssh-location" className="text-input" placeholder="Singapore" value={draft.location} onChange={(event) => setDraft({ ...draft, location: event.target.value })} /></label></div>{import.meta.env.DEV && <div className="dev-warning">Local development mode uses a fixed encryption key. Do not save real SSH credentials here.</div>}<div className="type-switch ssh-auth-switch" role="group" aria-label="SSH authentication method"><button type="button" className={sshAuthMethod === 'key' ? 'selected' : ''} onClick={() => { setSshAuthMethod('key'); setSshPassword('') }}><KeyRound size={14} />SSH key</button><button type="button" className={sshAuthMethod === 'password' ? 'selected' : ''} onClick={() => { setSshAuthMethod('password'); setSelectedIdentityId('') }}>Password</button></div>{sshAuthMethod === 'password' ? <><label className="field-label" htmlFor="ssh-password">SSH password<input id="ssh-password" className="text-input" type="password" value={sshPassword} onChange={(event) => setSshPassword(event.target.value)} autoComplete="off" maxLength={4096} required /></label><div className="security-note"><KeyRound size={15} /><span>Stored encrypted at rest; never shown again.</span></div></> : <>{sshIdentities.length > 0 ? <div className="saved-identity-picker"><label className="field-label" htmlFor="saved-ssh-key">Private key from SSH Files<select id="saved-ssh-key" className="text-input" value={sshIdentities.some((identity) => identity.id === selectedIdentityId) ? selectedIdentityId : ''} onChange={(event) => setSelectedIdentityId(event.target.value)} required><option value="">Choose a saved private key…</option>{sshIdentities.map((identity) => <option key={identity.id} value={identity.id}>{identity.name} · {identity.keyFingerprint}</option>)}</select></label><button type="button" className="secondary-button" onClick={() => void openSshLibrary()}><FolderOpen size={14} />Manage SSH Files</button></div> : <div className="ssh-library-empty"><p>No reusable private keys are saved.</p><button type="button" className="secondary-button" onClick={() => void openSshLibrary()}><FolderOpen size={14} />Open SSH Files to upload a key</button></div>}
{selectedIdentityId && <div className="security-note"><KeyRound size={15} /><span>This saved key stays encrypted on the server and will be attached to this VPS.</span></div>}</>}</> : <><div className="field-row"><label className="field-label" htmlFor="resource-provider">Provider<input id="resource-provider" className="text-input" placeholder="Cloudflare, Docker, AWS…" value={draft.provider} onChange={(event) => setDraft({ ...draft, provider: event.target.value })} /></label><label className="field-label" htmlFor="resource-endpoint">Endpoint<input id="resource-endpoint" className="text-input" placeholder="https://… or localhost:port" value={draft.endpoint} onChange={(event) => setDraft({ ...draft, endpoint: event.target.value })} /></label></div>{vpsNodes.length > 0 && <label className="field-label" htmlFor="resource-parent">Connected to <span className="optional-label">OPTIONAL</span><select id="resource-parent" className="text-input" value={draft.parentId} onChange={(event) => setDraft({ ...draft, parentId: event.target.value })}><option value="">No host link yet</option>{vpsNodes.map((node) => <option key={node.id} value={node.id}>{node.data.name}</option>)}</select></label>}<label className="field-label" htmlFor="resource-notes">Notes <span className="optional-label">OPTIONAL</span><textarea id="resource-notes" className="text-input notes-input" placeholder="Where it runs, which service it routes to, or other context." value={draft.notes} onChange={(event) => setDraft({ ...draft, notes: event.target.value })} /></label><div className="security-note"><Cloud size={15} /><span>Tunnels and services are saved as topology records. This does not query their provider APIs or monitor health.</span></div></>}
      <footer className="form-footer"><span>ADDED TO YOUR LOCAL MAP</span><button type="button" className="secondary-button" onClick={closeAdd} disabled={saveBusy}>Cancel</button><button type="submit" className="primary-button" disabled={saveBusy || (draft.type === 'vps' && (sshAuthMethod === 'password' ? !sshPassword : !sshIdentities.some((identity) => identity.id === selectedIdentityId)))}>{saveBusy ? <LoaderCircle size={16} className="spin" /> : <Plus size={15} />} Add resource</button></footer>
    </form></WindowFrame>}

    {editOpen && <WindowFrame desktop={desktop} title="Edit resource" onClose={() => { if (!saveBusy) { setEditOpen(false); setDetailsOpen(true); restoreWindow('details') } }} focusToken={windowFocus.edit || 0} minimized={minimizedWindows.includes('edit')} onMinimize={() => minimizeWindow('edit')}><form className="resource-modal" onSubmit={(event) => void saveEdit(event)}>
      <header className="modal-heading"><div><span className="modal-kicker">EDIT RESOURCE · {draft.type.toUpperCase()}</span><h2>Update this resource</h2><p>Change its saved connection details. Map links stay as they are.</p></div><button type="button" className="modal-close" onClick={() => { if (!saveBusy) { setEditOpen(false); setDetailsOpen(true); restoreWindow('details') } }} aria-label="Close edit form"><X size={17} /></button></header>
      <label className="field-label" htmlFor="edit-resource-name">Name<input id="edit-resource-name" className="text-input" value={draft.name} onChange={(event) => setDraft({ ...draft, name: event.target.value })} maxLength={180} required autoFocus /></label>
      {draft.type === 'vps' ? <>
        <div className="field-row"><label className="field-label" htmlFor="edit-ssh-host">Host / IP<input id="edit-ssh-host" className="text-input" value={draft.host} onChange={(event) => setDraft({ ...draft, host: event.target.value })} autoComplete="off" required /></label><label className="field-label port-field" htmlFor="edit-ssh-port">SSH port<input id="edit-ssh-port" className="text-input" inputMode="numeric" value={draft.port} onChange={(event) => setDraft({ ...draft, port: event.target.value })} required /></label></div>
        <div className="field-row"><label className="field-label" htmlFor="edit-ssh-user">SSH username<input id="edit-ssh-user" className="text-input" value={draft.username} onChange={(event) => setDraft({ ...draft, username: event.target.value })} autoComplete="off" required /></label><label className="field-label" htmlFor="edit-ssh-location">Location <span className="optional-label">OPTIONAL</span><input id="edit-ssh-location" className="text-input" value={draft.location} onChange={(event) => setDraft({ ...draft, location: event.target.value })} /></label></div>
        <label className="field-label" htmlFor="edit-resource-notes">Notes <span className="optional-label">OPTIONAL</span><textarea id="edit-resource-notes" className="text-input notes-input" value={draft.notes} onChange={(event) => setDraft({ ...draft, notes: event.target.value })} /></label>
        <div className="security-note"><KeyRound size={15} /><span>The saved SSH credentials stay attached. Private keys are managed in SSH Files.</span></div>
      </> : <>
        <div className="field-row"><label className="field-label" htmlFor="edit-resource-provider">Provider<input id="edit-resource-provider" className="text-input" value={draft.provider} onChange={(event) => setDraft({ ...draft, provider: event.target.value })} /></label><label className="field-label" htmlFor="edit-resource-endpoint">Endpoint<input id="edit-resource-endpoint" className="text-input" value={draft.endpoint} onChange={(event) => setDraft({ ...draft, endpoint: event.target.value })} /></label></div>
        <label className="field-label" htmlFor="edit-resource-notes">Notes <span className="optional-label">OPTIONAL</span><textarea id="edit-resource-notes" className="text-input notes-input" value={draft.notes} onChange={(event) => setDraft({ ...draft, notes: event.target.value })} /></label>
      </>}
      <footer className="form-footer"><span>GRAPH LINKS STAY ON THE MAP</span><button type="button" className="secondary-button" onClick={() => { if (!saveBusy) { setEditOpen(false); setDetailsOpen(true); restoreWindow('details') } }} disabled={saveBusy}>Cancel</button><button type="submit" className="primary-button" disabled={saveBusy}>{saveBusy ? <LoaderCircle size={16} className="spin" /> : <Check size={15} />} Save changes</button></footer>
    </form></WindowFrame>}
    {detailsOpen && selected && <WindowFrame desktop={desktop} title={selected.data.name} onClose={() => { setDetailsOpen(false); setDeleteConfirm(false) }} focusToken={windowFocus.details || 0} minimized={minimizedWindows.includes('details')} onMinimize={() => minimizeWindow('details')}><section className="details-modal"><header className="modal-heading"><div><span className="modal-kicker">{selected.data.kind}</span><h2>{selected.data.name}</h2><p>{selected.data.meta}</p></div><button className="modal-close" onClick={() => { setDetailsOpen(false); setDeleteConfirm(false) }} aria-label="Close"><X size={17} /></button></header>
      <div className="detail-grid"><span>TYPE</span><strong>{selected.data.resourceType.toUpperCase()}</strong>{selected.data.provider && <><span>PROVIDER</span><strong>{selected.data.provider}</strong></>}{selected.data.endpoint && <><span>ENDPOINT</span><strong className="detail-value">{selected.data.endpoint}</strong></>}{selected.data.resourceType === 'vps' && <><span>SSH TARGET</span><strong>{selected.data.username}@{selected.data.host}:{selected.data.port}</strong><span>SSH AUTH</span><strong>{selected.data.sshAuthMethod === 'password' ? 'Password · encrypted on this server' : selected.data.hasPrivateKey ? 'Private key · encrypted on this server' : 'No credentials saved'}</strong>{selected.data.hostFingerprint && <><span>HOST KEY</span><strong className="detail-value">{selected.data.hostFingerprint}</strong></>}{selected.data.lastCheckedAt && <><span>LAST CHECK</span><strong>{new Date(selected.data.lastCheckedAt).toLocaleString()}</strong></>}</>}{selected.data.notes && <><span>NOTES</span><strong className="detail-value">{selected.data.notes}</strong></>}</div>
      ...
      {deleteConfirm ? <div className="delete-confirm"><span>Removing this resource also deletes its encrypted SSH credentials and map links.</span><div><button className="secondary-button" onClick={() => setDeleteConfirm(false)}>Keep it</button><button className="danger-button" onClick={() => void deleteNode()}><Trash2 size={14} /> Remove resource</button></div></div> : <div className="detail-actions"><span>{selected.data.resourceType === 'tunnel' || selected.data.resourceType === 'service' || selected.data.resourceType === 'external' ? 'Inventory only · no health probe configured' : selected.data.hostFingerprint ? 'Pinned host key · first-use verification complete' : 'Host key verification required on first check'}</span><button type="button" className="secondary-button" disabled={isDemo} onClick={openEdit}><Pencil size={14} /> Edit</button><button type="button" className="danger-quiet" disabled={isDemo} onClick={() => setDeleteConfirm(true)}><Trash2 size={14} /> Remove</button></div>}
    </section></WindowFrame>}
