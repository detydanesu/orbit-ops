...
import { SshConfigUpload, type StoredSshProfile } from './SshConfigImport'
import SshKeyUpload from './SshKeyUpload'
import './SshLibrary.css'
...
export default function SshLibrary({ desktop, minimized, focusToken, profiles, identities, onClose, onMinimize, onSaveProfileContext, onSaveIdentityContext, onDeleteProfile, onDeleteIdentity, onDeleteAttachedKey, onUploadProfile, onUploadIdentity }: {
  ...
  onUploadProfile: (configName: string, uploadFileName: string, profiles: Omit<StoredSshProfile, 'id' | 'configName' | 'uploadFileName' | 'context'>[]) => Promise<void>
  onUploadIdentity: (name: string, privateKey: string, passphrase: string, uploadFileName: string) => Promise<void>
}) {
const [profileNotes, setProfileNotes] = useState<Record<string, string>>(() => Object.fromEntries([...new Set(profiles.map((profile) => profile.configName))].map((name) => [name, profiles.find((profile) => profile.configName === name)?.context || ''])));
  const [identityNotes, setIdentityNotes] = useState<Record<string, string>>(() => Object.fromEntries(identities.map((identity) => [identity.source + ':' + identity.id, identity.context || ''])));
  ...
  return <WindowFrame desktop={desktop} title="SSH File Library" onClose={onClose} onMinimize={onMinimize} focusToken={focusToken} minimized={minimized} wide>
    <section className="ssh-library-view">
      ...
      <p className="key-help">Upload SSH files here. Config text is parsed in your browser; private keys stay encrypted and are never displayed.</p>
      ...
      <section className="ssh-library-section"><h3><FileText size={17} /> SSH config files <small>{configNames.length}</small></h3>
        <SshConfigUpload onSave={onUploadProfile} />
        {configNames.length === 0 && <p className="key-help">No config files saved yet.</p>}
      ...
      <section className="ssh-library-section"><h3><FileKey2 size={17} /> Private-key files <small>{identities.length}</small></h3>
        <SshKeyUpload onSave={onUploadIdentity} />
        {identities.length === 0 && <p className="key-help">No private keys saved yet. Keys saved for reuse and keys stored on a VPS appear here.</p>}
